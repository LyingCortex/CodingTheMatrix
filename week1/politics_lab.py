...
def create_voting_dict(strlist):
    ...
    """
    return {x[0] : [int(y) for y in x[3:]] for x in [line.split() for line in strlist]}
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    return sum([u*v for (u,v) in zip(voting_dict[sen_a],voting_dict[sen_b])])
...
def most_similar(sen, voting_dict):
    ...
    """
    ll_max = max([policy_compare(sen,u,voting_dict) for (u,v) in voting_dict.items() if sen != u])
    for (u,v) in voting_dict.items():
        if u!=sen:
            if policy_compare(sen,u,voting_dict) == ll_max:
                return u
...
def least_similar(sen, voting_dict):
    ...
    Example:
        ...
        'b'
    """
    ll_min = min([policy_compare(sen,u,voting_dict) for (u,v) in voting_dict.items() if sen != u])
    for (u,v) in voting_dict.items():
        if u!=sen:
            if policy_compare(sen,u,voting_dict) == ll_min:
                return u
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        ...
        True
    """
    return sum( [policy_compare(sen,u,voting_dict) for u in sen_set]) / len(sen_set)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example: 
        ...
        [0.0, 1.0, 1.0]
    """
    dd =[0 for x in range(len(list(voting_dict.values())[0]))] 
    for sen  in sen_set:
        dd = [sum(x) for x in zip(voting_dict[sen],dd)]
    return [x/len(sen_set) for x in dd]



set_Democry = {x[0] : [int(y) for y in x[3:]] for x in [line.split() for line in list(open('voting_record_dump109.txt'))] if x[1] == 'D'}


average_Democrat_record = find_average_record({u for (u,v) in set_Democry.items()},set_Democry)  # give the vector as a list
...
def bitter_rivals(voting_dict):
    ...
    """
    M = float('infinity')
    sen_name = [u for (u,v) in voting_dict.items()]
    x = ''
    y = ''
    for sen_a in sen_name:
