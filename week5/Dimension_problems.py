...
def morph(S, B):
    ...
    A=set()
    for x in B:
        result.append=((x,exchange(list2vec(S)),B,x)
    return result
    '''
...
def direct_sum_decompose(U_basis, V_basis, w):
    ...
    for u in U_basis:
        for v in V_basis:
            if U_matrix*solve(U_matrix,u)+V_matrix*solve(V_matrix,v) == w:
                return (u,v)
    '''
