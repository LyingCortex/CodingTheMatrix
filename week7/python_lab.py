...
S = {-4, -2, 1, 2, 5, 0}
# Replace [ ...  with a one-line list comprehension in which S appears
exclude_zero_list = [(x,y,z) for x in S for y in S for z in S if x|y|z if x+y+z == 0 ]
...
first_of_tuples_list =[ (x,y,z) for x in S for y in S for z in S if x or y or z if x+y+z == 0 ][0]
...
odd_num_list_range = {i for i in range(100) if i%2}
...
representation_dict = { x:(x//base//base,x//base%base,x%base) for x in range(base ** 3) }
